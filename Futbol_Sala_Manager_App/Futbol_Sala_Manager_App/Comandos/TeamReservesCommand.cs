    ...
    public class TeamReservesCommand : ICommandShowable
    {
        ...
        {            
            if (!_visible)
            {
                for (int i = 0; i < n; i++)
                {
                    string peticion = "TeamReservesIN(['" + idioma[i].Reserves + "', '" + _equipo.TeamCode + "', '" +
                        idioma[i].Coach + " " + _equipo.Entrenador.FullName.Replace("'", "\\'") + "', '" + _equipo.Entrenador.ShortName.Replace("'", "\\'") + "'";
        ...
        private string genPeticionJugadores(IdiomaData idioma)
        {
            ...
            foreach (Jugador j in _equipo.Banquillo)
            {
                ...
                s += ", ['" + j.Number + "', '" + j.FullName.Replace("'", "\\'") + p + c + "', '" + j.ShortName.Replace("'", "\\'") + "']";
            }
