    ...
    public class ScoreBoardCommand : ICommandShowable
    {
        ...
        private string genPeticion(IdiomaData idioma)
        {
            ...
            foreach (Jugador jugador in jugadoresL)
            {
                ...
                {
                    lista.Add(new Gol(gol, jugador.ShortName.Replace("'", "\\'"), true, Gol.NORMAL, idioma));
                }
                foreach (Momento gol in jugador.GolesPenalty)
                {
                    lista.Add(new Gol(gol, jugador.ShortName.Replace("'", "\\'"), true, Gol.PENALTY, idioma));
                }
                foreach (Momento gol in jugador.GolesPP)
                {
                    lista.Add(new Gol(gol, jugador.ShortName.Replace("'", "\\'"), true, Gol.PP, idioma));
                }
            }
            ...
            foreach (Jugador jugador in jugadoresV)
            {
                foreach (Momento gol in jugador.Goles)
                {
                    lista.Add(new Gol(gol, jugador.ShortName.Replace("'", "\\'"), false, Gol.NORMAL, idioma));
                }
                foreach (Momento gol in jugador.GolesPenalty)
                {
                    lista.Add(new Gol(gol, jugador.ShortName.Replace("'", "\\'"), false, Gol.PENALTY, idioma));
                }
                foreach (Momento gol in jugador.GolesPP)
                {
                    lista.Add(new Gol(gol, jugador.ShortName.Replace("'", "\\'"), false, Gol.PP, idioma));
                }
            }

            // Ordena los goles por el momento en el que se han marcado
            lista.Sort();
            //lista.Sort((x, y) => x..CompareTo(y.OrderDate));
        ...
        class Gol : IComparable<Gol>
        {
            ...
            public Gol(Momento momento, string jugador, bool local, int tipo, IdiomaData idioma)
            {
                ...
                this.segundo = momento.GetSegundoAbsoluto();
