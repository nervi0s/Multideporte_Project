    ...
    public class EstadisticasCommand : ICommandShowable
    {
                ...
                for (int i = 0; i < n; i++)
                {
                    if (Program.EstaActivado(i))
                        if (_nEstadisticas == 2)
                        {
                            ...
                            {
                                ipf[i].Envia("Statistics2linesIN(['" + statNameV2(_stat, idioma[i]) + "', '" +
                                getStat(_stat, _equipoL) + "','" + getStat(_stat, _equipoV) + "', '" + _equipoL.TeamCode + "','" + _equipoV.TeamCode + "']"
                                + ","
                                + "['" + statNameV2(_stat2, idioma[i]) + "', '" + getStat(_stat2, _equipoL) + "','" + getStat(_stat2, _equipoV) + "', '" + _equipoL.TeamCode + "','" + _equipoV.TeamCode + "']" + ")");
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            if (AttemptsAndOnTarget())
                            {
                                ipf[i].Envia("Statistics3linesIN(['" + statNameV2(_stat, idioma[i]) + "', '" +
                                getStat(_stat, _equipoL) + "','" + getStat(_stat, _equipoV) + "', '" + _equipoL.TeamCode + "','" + _equipoV.TeamCode + "']"
                                + ","
                                + "['" + statNameV2(_stat2, idioma[i]) + "', '" + getStat(_stat2, _equipoL) + "','" + getStat(_stat2, _equipoV) + "', '" + _equipoL.TeamCode + "','" + _equipoV.TeamCode + "']" +
                                ","
                                + "['" + statNameV2(_stat3, idioma[i]) + "', '" + getStat(_stat3, _equipoL) + "','" + getStat(_stat3, _equipoV) + "', '" + _equipoL.TeamCode + "','" + _equipoV.TeamCode + "']" +
                                ")");
                            }
                            else
                            {
        ...
        private string getStat(int stat, Equipo equipo)
        {
            switch (stat)
            {
                ...
                case Faltas:
                    return equipo.getFaltas().ToString();
        ...
        private string statNameV2(int stat, IdiomaData idioma)
        {
            switch (stat)
            {
                case posesiones:
