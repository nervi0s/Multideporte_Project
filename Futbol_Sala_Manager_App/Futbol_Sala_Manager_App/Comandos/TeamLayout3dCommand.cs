...
namespace Futbol_Sala_Manager_App.Comandos
{
    public class TeamLayout3dCommand : ICommandShowable
    {
        ...
        private string genPeticionJugadores()
        {
            string s = "";
            ...
            foreach (Jugador j in _equipo.Jugadores)
            {
                Console.WriteLine(String.Format("{0,2} {1,-20} [{2:+0.00;-0.00} ; {3:+0.00;-0.00}] -> [{4:0.00} ; {5:0.00}]",
                    j.Number, j.ShortName.Replace("'", "\\'"), (Convert.ToSingle(j.PosX) * 2 - 1), (Convert.ToSingle(j.PosY) * 2 - 1), Convert.ToSingle(j.PosX), Convert.ToSingle(j.PosY)));
