...
namespace Futbol_Sala_Manager_App.AccesoBD
{
    ...
    public class AccessData : IAccesoBD
    {
        ...
        private Equipo getEquipo(int idEncuentro, bool local)
        {
            ...
            //equipo.Campo = Convert.ToString(dataReader["nombre_campo"]);
            //equipo.Ciudad = Convert.ToString(dataReader["ciudad"]);
            equipo.Local = local;
        ...
        private List<Jugador> getJugadores(int idEncuentro, bool local, bool titular)
        {
            ...
            while (dataReader.Read())
            {
                ...
                string posicionStr = (string)dataReader["posicion"];
                if (posicionStr == "Portero")
                    jugador.Posicion = Jugador.Portero;
                else if (posicionStr == "Cierre")
                    jugador.Posicion = Jugador.Cierre;
                else if (posicionStr == "Ala Cierre")
                    jugador.Posicion = Jugador.AlaCierre;
                else if (posicionStr == "Ala")
                    jugador.Posicion = Jugador.Ala;
                else if (posicionStr == "AlaPivot")
                    jugador.Posicion = Jugador.AlaPivot;
                else if (posicionStr == "Pivot")
                    jugador.Posicion = Jugador.Pivot;
                else
                    jugador.Posicion = Jugador.Universal;
        ...
        private List<ICommand> getArbitros(int idEncuentro)
        {
            ...
            {
                SQLiteDataReader dataReader =
                     ejecutarSelect("SELECT arbitros.*, 1 AS colegio FROM encuentros, arbitros WHERE (encuentros.id_arbitro1 = arbitros.id) and (encuentros.id=" + idEncuentro + ") " +
                              "UNION SELECT arbitros.*, 2 AS colegio FROM encuentros, arbitros WHERE (encuentros.id_arbitro2 = arbitros.id) and (encuentros.id=" + idEncuentro + ");");
                ...
                arbitroDummy.Cargo = Arbitro.arbitro1;
