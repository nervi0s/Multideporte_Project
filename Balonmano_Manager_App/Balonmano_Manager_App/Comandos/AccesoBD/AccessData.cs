...
namespace Balonmano_Manager_App.AccesoBD
{
    /**
    ...
    public class AccessData : IAccesoBD
    {
        ...
        private Equipo getEquipo(int idEncuentro, bool local)
        {
            ...
            equipo.EntrenadorAsistente = new Jugador();
            equipo.EntrenadorAsistente.FullName = Convert.ToString(dataReader["nombre_entrenador_asistente"]);
            equipo.EntrenadorAsistente.ShortName = Convert.ToString(dataReader["nombre_corto_entrenador_asistente"]);
            equipo.EntrenadorAsistente.RutaFoto = Convert.ToString(dataReader["ruta_foto_entrenador_asistente"]);
            equipo.EntrenadorAsistente.Equipo = equipo;
        ...
        private List<Jugador> getJugadores(int idEncuentro, bool local, bool titular)
        {
            ...
            while (dataReader.Read())
            {
                ...
                jugador.Dorsal_Interno = i;

                string posicionStr = Convert.ToString(dataReader["posicion"]);
                if (posicionStr == "Portero")
                    jugador.Posicion = Jugador.Portero;
                else if (posicionStr == "Extremo")
                    jugador.Posicion = Jugador.Extremo;
                else if (posicionStr == "Extremo Derecho")
                    jugador.Posicion = Jugador.ExtremoDerecho;
                else if (posicionStr == "Extremo Izquierdo")
                    jugador.Posicion = Jugador.ExtremoIzquierdo;
                else if (posicionStr == "Lateral")
                    jugador.Posicion = Jugador.Lateral;
                else if (posicionStr == "Lateral Derecho")
                    jugador.Posicion = Jugador.LateralDerecho;
                else if (posicionStr == "Lateral Izquierdo")
                    jugador.Posicion = Jugador.LateralIzquierdo;
                else if (posicionStr == "Central")
                    jugador.Posicion = Jugador.Central;
                else
                    jugador.Posicion = Jugador.Pivote;

                lista.Add(jugador);
                i++;

                //Console.WriteLine(jugador.Number + " " + jugador.FullName + " > " + posicionStr + "(" + jugador.Posicion + ")");
            }
        ...
        private List<ICommand> getArbitros(int idEncuentro)
        {
            try
            {
               SQLiteDataReader dataReader =
                ...
                Arbitro arbitro1 = null, arbitro2 = null;
                ...
                while (dataReader.Read())
                {
                    ...
                    arbitro.Cargo = Convert.ToInt32(dataReader["cargo"]);                    
                    if (indice == 0)
                ...
                if (arbitro1 != null && arbitro2 != null)
                    arbitros.Insert(0, new RefereesCommand(arbitro1, arbitro2));
        ...
        private List<ICommand> getComentaristas(int idEncuentro)
        {
            try
            {
                ...
                while (dataReader.Read())
                {
                    comentaristas.Add(new FreeTextCommand(Convert.ToString(dataReader["nombre"]), Convert.ToString(dataReader["nombre_corto"]), Convert.ToString(dataReader["cargo"])));
                }
        ...
        private string getAgregate(int idEncuentro)
        {
            ...
            string agregate = Convert.ToString(dataReader["resultado_ida"]);
